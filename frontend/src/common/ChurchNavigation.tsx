  ...
  return (
    <header className="w-full h-30">
      <nav className=" w-full h-30 flex justify-between items-center px-10 bg-white shadow-md">
          ...
          {NavLinks.map((link) => (
            <li key={link.id}>
              <NavLink
                className={(isActive) =>
                  isActive
                    ? "text-church-base text-church-primary font-semibold mx-6"
        ...
        <div>
          <Link to="/donation">
            <button className="bg-church-sec text-white px-4 py-2 rounded-md hover:bg-gray-600 transition duration-300">
              Donate
            </button>
          </Link>
        </div>
